...
function [obj, tracks, full, FrameWithTracker] = PedestrianTracker()
...
option.ROI                  = [100 200 1500 500];  % A rectangle [x, y, w, h] that limits the processing area to ground locations.
...
for fNum = 1:stopFrame
    ...
    full = updateAssignedTracks();    
    updateUnassignedTracks();    
    deleteLostTracks();    
    createNewTracks();
    
    FrameWithTracker = displayTrackingResults(full);
...
% based on the previous detection scores. 
    ...
    function density = updateAssignedTracks()
        numAssignedTracks = size(assignments, 1);
        counter = 1;
        if numAssignedTracks > 9
            density = 1;
            disp("Room looks full, leave!")
            counter = counter + 1
        else
            density = 0;
            %disp("We chillin")
...
% detections and tracks.
    ...
    function FrameWithTracker = displayTrackingResults(full)
        ...
        if full == 0
            frame = insertText(frame,[532, 26], 'Sparse enough for safety', 'FontSize',...
                36, 'BoxColor', 'green', 'TextColor', 'black');
        else
            frame = insertText(frame,[532, 26], 'Densley populated, should leave', 'FontSize',...
                36, 'BoxColor', 'red', 'TextColor', 'black');
        end
            
                            
        FrameWithTracker = frame;
